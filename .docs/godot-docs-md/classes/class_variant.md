 ...
 **Containers (Array and Dictionary):** Both are implemented using variants. A [Dictionary](class_dictionary.md#class_Dictionary) can match any datatype used as key to any other datatype. An [Array](class_array.md#class_Array) just holds an array of Variants. Of course, a Variant can also hold a [Dictionary](class_dictionary.md#class_Dictionary) and an [Array](class_array.md#class_Array) inside, making it even more flexible.

Modifications to a container will modify all references to it. A [Mutex](class_mutex.md#class_Mutex) should be created to lock it if multi-threaded access is desired.
