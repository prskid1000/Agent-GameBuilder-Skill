<span id="doc_custom_modules_in_cpp"></span>
...
> **Note:** While it is possible to use modules for custom game logic, :ref:`GDExtension <doc_gdextension>` is generally more suited as it doesn't require recompiling the engine after every code change. C++ modules are mainly needed when GDExtension doesn't suffice and deeper engine integration is required.
...
> **Warning:** Any path passed to ``custom_modules`` will be converted to an absolute path internally as a way to distinguish between custom and built-in modules. It means that things like generating module documentation may rely on a specific path structure on your machine.
.. seealso::

    [Introduction to the buildsystem - Custom modules build option](../development/compiling/introduction_to_the_buildsystem.md#doc_buildsystem_custom_modules).
...
> **Note:** ``MODULE_SUMMATOR_HAS_PREREGISTER`` to let the build system know what relevant method calls to include at compile time. The module's name has to be converted to uppercase as well.
```cpp
:caption: godot/modules/summator/register_types.cpp
```
...
<span id="doc_custom_module_unit_tests"></span>
...
[doc_unit_testing](../architecture/unit_testing.md#doc_unit_testing).
...
<span id="doc_custom_module_icons"></span>
...
please refer to [doc_editor_icons](../editor/creating_icons.md#doc_editor_icons) first.
...
-  If you inherit from [class_Node](../../classes/class_node.md#class_Node) (or any derived node type, such as
   Sprite2D), your new class will appear in the editor, in the inheritance
   tree in the "Add Node" dialog.
-  If you inherit from [class_Resource](../../classes/class_resource.md#class_Resource), it will appear in the resource
