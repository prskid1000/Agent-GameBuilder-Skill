...
For instance, you can use the open source Inkscape editor.
...
pre-commit hook runs, it automatically optimizes
the SVG using svgo.

> **Note:** While this optimization step won't impact the icon's quality noticeably, it will still remove editor-only information such as guides. Therefore, it's recommended to keep the source SVG around if you need to make further changes.
...
[Creating custom module icons](../engine_api/custom_modules_in_cpp.md#doc_custom_module_icons).
