<span id="doc_compiling_for_linuxbsd"></span>
...
.. seealso::
    ...
    If you're looking to export your project to Linux instead, read [doc_exporting_for_linux](../../../tutorials/export/exporting_for_linux.md#doc_exporting_for_linux).
...
.. seealso::
    ...
    To get the Godot source code for compiling, see [doc_getting_source](getting_source.md#doc_getting_source).

    For a general overview of SCons usage for Godot, see [doc_introduction_to_the_buildsystem](introduction_to_the_buildsystem.md#doc_introduction_to_the_buildsystem).

<span id="doc_compiling_for_linuxbsd_oneliners"></span>
...
> **Note:** Prior to Godot 4.0, the Linux/\*BSD target was called ``x11`` instead of ``linuxbsd``. If you are looking to compile Godot 3.x, make sure to use the `3.x branch of this documentation <https://docs.godotengine.org/en/3.6/development/compiling/compiling_for_x11.html>`__.
.. tip::
...
> **Note:** If you wish to compile using Clang rather than GCC, use this command: :: scons platform=linuxbsd use_llvm=yes Using Clang appears to be a requirement for OpenBSD, otherwise fonts would not build. For RISC-V architecture devices, use the Clang compiler instead of the GCC compiler.
> **Note:** Compiling on some platforms such as OpenBSD may require more memory than available by default. To increase the memory limit on OpenBSD within the maximum for the current user, run ``ulimit -d {new amount in kB}``.
...
> **Note:** the SO to ``accesskit.{architecture}.so`` and place them alongside the export template executables, and the libraries will be automatically copied during the export process.
...
[remote debugging tools](../../../tutorials/editor/command_line_tutorial.md#doc_command_line_tutorial), use:
...
> **Warning:** older than the distribution they were built on. If you wish to distribute binaries that work on most distributions, you should build them on an old distribution such as Ubuntu 20.04. You can use a virtual machine or a container to set up a suitable build environment.
...
- Follow these instructions to configure, build, and
...
For even faster linking compared to LLD, you can use mold.
...
Godot bundles the source code of various third-party libraries.
...
listed in the [doc_compiling_for_linuxbsd_oneliners](compiling_for_linuxbsd.md#doc_compiling_for_linuxbsd_oneliners):
...
> **Warning:** When using system libraries, the resulting binary is **not** portable across Linux distributions anymore. Do not use this approach for creating binaries you intend to distribute to others, unless you're creating a package for a Linux distribution.
