<span id="doc_using_sanitizers"></span>
...
[doc_unit_testing](../../architecture/unit_testing.md#doc_unit_testing) in continuous integration.

Sanitizers can be used on Windows, macOS and Linux by using the Clang (LLVM),
GCC or Visual Studio compilers.
[Certain platforms](using_sanitizers.md#doc_using_sanitizers_platform_specific_sanitizers)
...
<span id="doc_using_sanitizers_address_sanitizer"></span>
...
the address sanitizer doesn't suffice. The [doc_using_sanitizers_memory_sanitizer](using_sanitizers.md#doc_using_sanitizers_memory_sanitizer)
...
> **Warning:** Due to a `design decision <https://stackoverflow.com/questions/36971902/why-cant-clang-enable-all-sanitizers/>`__, the address, memory and thread sanitizers are mutually exclusive. This means you can only use one of those sanitizers in a given binary.
### Leak sanitizer (LSAN)
...
[dedicated servers](../../../tutorials/export/exporting_for_dedicated_servers.md#doc_exporting_for_dedicated_servers) for months or
...
<span id="doc_using_sanitizers_memory_sanitizer"></span>
...
[doc_using_sanitizers_address_sanitizer](using_sanitizers.md#doc_using_sanitizers_address_sanitizer). Unlike the address sanitizer,
...
<span id="doc_using_sanitizers_platform_specific_sanitizers"></span>
...
When [compiling for the Web](../compiling/compiling_for_web.md#doc_compiling_for_web),
...
- `use_assertions=yes` enables runtime Emscripten assertions, which can catch
  various issues.
- `use_safe_heap=yes` enables Emscripten's SAFE_HEAP sanitizer.
