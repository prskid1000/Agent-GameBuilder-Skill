<span id="doc_first_3d_game_jumping_and_squashing_monsters"></span>
...
the [CharacterBody3D](../../classes/class_characterbody3d.md#class_CharacterBody3D) class and are related to
`move_and_slide()`.

`get_slide_collision()` returns a
[KinematicCollision3D](../../classes/class_kinematiccollision3d.md#class_KinematicCollision3D) object that holds
...
> **Note:** The method ``is_in_group()`` is available on every :ref:`Node<class_Node>`.
...
> **Note:** When using C#, Godot will create the appropriate events automatically for all Signals ending with `EventHandler`, see :ref:`C# Signals <doc_c_sharp_signals>`.
