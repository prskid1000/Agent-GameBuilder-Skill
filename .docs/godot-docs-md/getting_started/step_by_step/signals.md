...
Like methods ([class_callable](../../classes/class_callable.md#class_callable)), signals are a first-class type since Godot
...
less error-prone. See the [class_signal](../../classes/class_signal.md#class_signal) class reference for a list of
...
([doc_scripting_player_input](scripting_player_input.md#doc_scripting_player_input)) move and stop by pressing a button.

> **Note:** - **GDScript**: Classes (nodes) use PascalCase, variables and functions use snake_case, and constants use ALL_CAPS (See :ref:`doc_gdscript_styleguide`). - **C#**: Classes, export variables and methods use PascalCase, private fields use _camelCase, local variables and parameters use camelCase (See :ref:`doc_c_sharp_styleguide`). Be careful to type the method names precisely when connecting signals.
### Scene setup

To add a button to our game, we will create a new scene which will include
both a [Button](../../classes/class_button.md#class_button) and the `sprite_2d.tscn` scene we created in
the [doc_scripting_first_script](scripting_first_script.md#doc_scripting_first_script) lesson.

Create a new scene by going to the menu :menu:`Scene > New Scene`.

![](img/signals_01_new_scene.webp)

In the Scene dock, click the :button:`2D Scene` button. This will add
a [Node2D](../../classes/class_node2d.md#class_Node2D) as our root.

![](img/signals_02_2d_scene.webp)

In the FileSystem dock, click and drag the `sprite_2d.tscn` file you saved
previously onto the Node2D to instantiate it.

![](img/signals_03_dragging_scene.webp)

We want to add another node as a sibling of the Sprite2D. To do so, right-click
on Node2D and select :button:`Add Child Node`.

![](img/signals_04_add_child_node.webp)

Search for the [Button](../../classes/class_button.md#class_button) node and add it.

![](img/signals_05_add_button.webp)
...
![](img/signals_07_select_tool.webp)
...
![](img/signals_08_toggle_motion_text.webp)

Your scene tree and viewport should look like this.

![](img/signals_09_scene_setup.webp)
...
![](img/signals_advanced_connection_window.webp)
...
> **Note:** If you are using an external editor (such as VS Code), this automatic code generation might not work. In this case, you need to connect the signal via code as explained in the next section.
...
![](img/signals_13_signals_connection_icon.webp)
...
![](img/signals_14_signals_connection_info.webp)
...
Let's use a different node here. Godot has a [Timer](../../classes/class_timer.md#class_Timer) node
...
> **Note:** method of the signal you want to listen to. In this case, we want to listen to the Timer's "timeout" signal.
We want to connect the signal when the scene is instantiated, and we can do that
using the [Node._ready()](../../classes/class_node.md#class_Node_private_method__ready) built-in function,
which is called automatically by the engine when a node is fully instantiated.

To get a reference to a node relative to the current one, we use the method
[Node.get_node()](../../classes/class_node.md#class_Node_method_get_node). We can store the reference
in a variable.
...
> **Note:** "_on_node_name_signal_name" and in C# as "OnNodeNameSignalName". Here, it'll be "_on_timer_timeout" for GDScript and OnTimerTimeout() for C#.
.. tabs::
```
```
...
> **Note:** and does not build upon the project created in previous lessons.
...
> **Note:** action verb in the past tense in their names.
...
![](img/signals_17_custom_signal.webp)
...
For example, an [Area2D](../../classes/class_area2d.md#class_Area2D) representing a coin emits a
...
In the next section, [doc_your_first_2d_game](../first_2d_game/index.md#doc_your_first_2d_game), you'll create a complete 2D
