..
    ...
    - We assume the reader has programming foundations. If you don't, consider
      taking the course we recommend in the [introduction to Godot page](../introduction/introduction_to_godot.md#doc_introduction_learning_programming).
...
<span id="doc_scripting_first_script"></span>
...
.. seealso:: To learn more about GDScript, its keywords, and its syntax, head to
             the [doc_gdscript](../../tutorials/scripting/gdscript/index.md#doc_gdscript) section. To learn more about C#,
             head to the [doc_c_sharp](../../tutorials/scripting/c_sharp/index.md#doc_c_sharp) section.

### Project setup

Please [create a new project](../../tutorials/editor/project_manager.md#doc_creating_and_importing_projects) to
...
![](img/scripting_first_script_setting_texture.webp)
...
![](img/scripting_first_script_centering_sprite.webp)
...
![](img/scripting_first_script_attach_script.webp)
...
![](img/scripting_first_script_attach_node_script.webp)

> **Note:** C# script names need to match their class name. In this case, you should name the file ``MySprite2D.cs``.
...
> **Note:** class will implicitly extend :ref:`RefCounted <class_RefCounted>`, which Godot uses to manage your application's memory.
Inherited properties include the ones you can see in the :ui:`Inspector` dock, like
our node's `texture`.

> **Note:** By default, the :ui:`Inspector` displays a node's properties in "Title Case", with capitalized words separated by a space. In GDScript code, these properties are in "snake_case", which is lowercase with each word separated by an underscore. You can hover over any property's name in the :ui:`Inspector` to see a description and its identifier in code.
### Hello, world!
...
> **Note:** that says ``print()`` is necessary for the code to work. If you omit it or don't indent a line correctly, the editor will highlight it in red and display the following error message: "Indented block expected".
...
![](img/scripting_first_script_print_hello_world.webp)
...
> **Note:** but you have built-in functions and properties available if you prefer to calculate angles in degrees instead.
...
> **Note:** underscore. By convention, Godot's virtual functions, that is to say, built-in functions you can override to communicate with the engine, start with an underscore.
...
> **Note:** ``double``. We therefore need to convert it to ``float`` when we apply it to the rotation.
Moving forward
~~~~~~~~~~~~~~
...
itself is of type [Vector2](../../classes/class_vector2.md#class_Vector2), a built-in type in Godot
...
> **Note:** walls or the floor. In :ref:`doc_your_first_2d_game`, you will learn another approach to moving objects while detecting collisions.
Our node currently moves by itself. In the next part,
[doc_scripting_player_input](scripting_player_input.md#doc_scripting_player_input), we'll use player input to control it.
