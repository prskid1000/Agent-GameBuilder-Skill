<span id="doc_upgrading_to_godot_4"></span>
...
- 4.x will enjoy a longer [support period](../../about/release_policy.md#doc_release_policy). Godot 3.x
  will continue to be supported for some time after 4.0 is released, but it will
  eventually stop receiving support.

See [doc_docs_changelog](../../about/docs_changelog.md#doc_docs_changelog) for a list of pages documenting new features in
Godot 4.0, and [doc_list_of_features](../../about/list_of_features.md#doc_list_of_features) for a list of all features in Godot.
...
- Since Godot 4 includes more features than Godot 3, Godot 4's binary size for
  ...
  [optimizing a build for size](../../engine_details/development/compiling/optimizing_for_size.md#doc_optimizing_for_size), a 4.0 build with
  a given set of enabled modules will remain larger compared to a 3.x build with
  the same modules. This can be an issue for
  [exporting to the Web](../export/exporting_for_web.md#doc_exporting_for_web), as binary size directly
...
- By default, rendering in 2D is no longer performed in HDR, which means
  ...
  to perform 2D rendering in HDR. See also [doc_environment_and_post_processing_using_glow_in_2d](../3d/environment_and_post_processing.md#doc_environment_and_post_processing_using_glow_in_2d).
...
[class_Tweener](../../classes/class_tweener.md#class_Tweener) and the [class_Time](../../classes/class_time.md#class_Time) singleton in your project. These
...
To use the project upgrade tool from the [command line](../editor/command_line_tutorial.md#doc_command_line_tutorial),
...
<span id="doc_upgrading_to_godot_4_manual_rename"></span>
...
- File and Directory classes were replaced by [class_FileAccess](../../classes/class_fileaccess.md#class_FileAccess) and
  [class_DirAccess](../../classes/class_diraccess.md#class_DirAccess), which have an entirely different API. Several methods
...
- Screen and window-related methods from the [class_OS](../../classes/class_os.md#class_OS) singleton (such as
  `OS.get_screen_size()`) were moved to the [class_DisplayServer](../../classes/class_displayserver.md#class_DisplayServer) singleton.
...
- Time and date methods from the [class_OS](../../classes/class_os.md#class_OS) singleton were moved to the
  [class_Time](../../classes/class_time.md#class_Time) singleton.
...
- AnimationPlayer's `add_animation()` is now `add_animation_library()` and now uses an [class_AnimationLibrary](../../classes/class_animationlibrary.md#class_AnimationLibrary).
...
Godot 3.x, you will have to change its code to call [class_RenderingServer](../../classes/class_renderingserver.md#class_RenderingServer)
...
- [Built in matrix variables were renamed.](../shaders/shader_reference/spatial_shader.md#doc_spatial_shader)
...
See [doc_shading_language](../shaders/shader_reference/shading_language.md#doc_shading_language) for more information.
...
- Both [class_String](../../classes/class_string.md#class_String) and [class_StringName](../../classes/class_stringname.md#class_StringName) are now exposed to
...
- [GDScript setter and getter syntax](../scripting/gdscript/gdscript_basics.md#doc_gdscript_basics_setters_getters)
...
- [GDScript signal connection syntax](../scripting/gdscript/gdscript_basics.md#doc_gdscript_signals) was changed.
  The conversion tool will use the string-based syntax which is still present in
  Godot 4, but it's recommended to switch to the [class_Signal](../../classes/class_signal.md#class_Signal)-based syntax
...
- Built-in scripts that are [tool scripts](../plugins/running_code_in_the_editor.md#doc_running_code_in_the_editor)
...
- Instead of `rotation_degrees`, the `rotation` property is exposed to the
  ...
  conversion tool.
- [class_AABB](../../classes/class_aabb.md#class_AABB)'s `has_no_surface()` was inverted and renamed to `has_surface()`.
- [class_AABB](../../classes/class_aabb.md#class_AABB) and [class_Rect2](../../classes/class_rect2.md#class_Rect2)'s `has_no_area()` was inverted and
  renamed to `has_area()`.
- [class_AnimatedTexture](../../classes/class_animatedtexture.md#class_AnimatedTexture)'s `fps` property was replaced by `speed_scale`,
  which works the same as AnimationPlayer's `playback_speed` property.
- [class_AnimatedSprite2D](../../classes/class_animatedsprite2d.md#class_AnimatedSprite2D) and [class_AnimatedSprite3D](../../classes/class_animatedsprite3d.md#class_AnimatedSprite3D) now allow
  negative `speed_scale` values. This may break animations if you relied on
  `speed_scale` being internally clamped to `0.0`.
- [class_AnimatedSprite2D](../../classes/class_animatedsprite2d.md#class_AnimatedSprite2D) and [class_AnimatedSprite3D](../../classes/class_animatedsprite3d.md#class_AnimatedSprite3D)'s `playing`
...
- [class_Array](../../classes/class_array.md#class_Array)'s `slice()` second parameter (`end`) is now *exclusive*,
  instead of being inclusive. For example, this means that
  `[1, 2, 3].slice(0, 1)` now returns `[1]` instead of `[1, 2]`.
- [class_BaseButton](../../classes/class_basebutton.md#class_BaseButton)'s signals are now `button_up` and `button_down`.
  The `pressed` property is now `button_pressed`.
- [class_Camera2D](../../classes/class_camera2d.md#class_Camera2D)'s `rotating` property was replaced by
  `ignore_rotation`, which has inverted behavior.
- Camera2D's `zoom` property was inverted: higher values are now more zoomed
  in, instead of less.
- [class_Node](../../classes/class_node.md#class_Node)'s `remove_and_skip()` method was removed.
...
- [class_ResourceSaver](../../classes/class_resourcesaver.md#class_ResourceSaver)'s `save()` method now has its arguments swapped around
  (`resource: Resource, path: String`). This also applies to
  [class_ResourceFormatSaver](../../classes/class_resourceformatsaver.md#class_ResourceFormatSaver)'s `_save()` method.
- A [class_StreamPeerTCP](../../classes/class_streampeertcp.md#class_StreamPeerTCP) must have `poll()` called on it to update its
  state, instead of relying on `get_status()` automatically polling:
  GH-59582
- [class_String](../../classes/class_string.md#class_String)'s `right()` method has changed behavior:
...
- `is_connected_to_host()` was removed from StreamPeerTCP and PacketPeerUDP as
  ...
  `is_socket_connected()` can be used in [class_PacketPeerUDP](../../classes/class_packetpeerudp.md#class_PacketPeerUDP) instead.
...
- VisualShaderNodeVec4Parameter now takes a [class_Vector4](../../classes/class_vector4.md#class_Vector4) as parameter
  instead of a [class_Quaternion](../../classes/class_quaternion.md#class_Quaternion).
...
+---------------------+-----------------------+----------------------------------------------------------------------------+
| BakedLightmap       | LightmapGI            | See [doc_using_lightmap_gi](../3d/global_illumination/using_lightmap_gi.md#doc_using_lightmap_gi).                                          |
+---------------------+-----------------------+                                                                            |
| BakedLightmapData   | LightmapGIData        |                                                                            |
+---------------------+-----------------------+----------------------------------------------------------------------------+
| BitmapFont          | FontFile              | See [doc_gui_using_fonts](../ui/gui_using_fonts.md#doc_gui_using_fonts).                                            |
+---------------------+-----------------------+                                                                            |
...
+---------------------+-----------------------+----------------------------------------------------------------------------+
| Navigation2D        | Node2D                | Replaced by [other 2D Navigation nodes](../navigation/navigation_introduction_2d.md#doc_navigation_overview_2d). |
+---------------------+-----------------------+----------------------------------------------------------------------------+
| Navigation3D        | Node3D                | Replaced by [other 3D Navigation nodes](../navigation/navigation_introduction_3d.md#doc_navigation_overview_3d). |
+---------------------+-----------------------+----------------------------------------------------------------------------+
...
+---------------------+-----------------------+----------------------------------------------------------------------------+
| ProximityGroup      | Node3D                | [class_VisibleOnScreenNotifier3D](../../classes/class_visibleonscreennotifier3d.md#class_VisibleOnScreenNotifier3D) can act as a replacement.           |
+---------------------+-----------------------+----------------------------------------------------------------------------+
| Portal              | Node3D                | Portal and room occlusion culling was replaced by raster                   |
|                     |                       | [occlusion culling](../3d/occlusion_culling.md#doc_occlusion_culling)                           |
...
[Threading](../performance/using_multiple_threads.md#doc_using_multiple_threads) APIs have changed in 4.0. For
...
Lines that are commented out refer to API renames that [cannot be performed automatically](upgrading_to_godot_4.md#doc_upgrading_to_godot_4_manual_rename).
...
[editor settings folder](../io/data_paths.md#doc_data_paths_editor_data_paths) and copy
...
be ignored by your [version control system](../best_practices/version_control_systems.md#doc_version_control_systems).
