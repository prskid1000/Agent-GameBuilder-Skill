<span id="doc_setting_up_xr"></span>
...
At the core sits the [XRServer](../../classes/class_xrserver.md#class_xrserver) which acts as a central interface to the XR system that allows users to discover interfaces and interact with the components of the XR system.

Each supported XR platform is implemented as an [XRInterface](../../classes/class_xrinterface.md#class_xrinterface).
A list of supported platforms can be found on the list of features page [here](../../about/list_of_features.md#doc_xr_support).
Supported interfaces register themselves with the [XRServer](../../classes/class_xrserver.md#class_xrserver)
and can be queried with the `find_interface` method on the [XRServer](../../classes/class_xrserver.md#class_xrserver).
...
Due to the special requirements for output in XR, especially for head mounted devices that supply different images to each eye, the [XRServer](../../classes/class_xrserver.md#class_xrserver) in Godot will override various features in the rendering system. For stand-alone devices this means the final output is handled by the [XRInterface](../../classes/class_xrinterface.md#class_xrinterface) and Godot's usual output system is disabled. For desktop XR devices that work as a second screen it is possible to dedicate a separate [Viewport](../../classes/class_viewport.md#class_viewport) to handle the XR output, leaving the main Godot window available for displaying alternative content.
...
- [XROrigin3D](../../classes/class_xrorigin3d.md#class_xrorigin3d) represents, for all intents and purposes, the center point of your play space. That is an oversimplified statement but we'll go into more detail later. All objects tracked in physical space by the XR platform are positioned in relation to this point.
- [XRCamera3D](../../classes/class_xrcamera3d.md#class_xrcamera3d) represents the (stereo) camera that is used when rendering output for the XR device. The positioning of this node is controlled by the XR system and updated automatically using the tracking information provided by the XR platform.
- [XRController3D](../../classes/class_xrcontroller3d.md#class_xrcontroller3d) represents a controller used by the player, commonly there are two, one held in each hand. These nodes give access to various states on these controllers and send out signals when the player presses buttons on them. The positioning of this node is controlled by the XR system and updated automatically using the tracking information provided by the XR platform.
...
but for more information on what's here see [doc_openxr_settings](openxr_settings.md#doc_openxr_settings).
...
Every XR application needs at least an [XROrigin3D](../../classes/class_xrorigin3d.md#class_xrorigin3d) and an [XRCamera3D](../../classes/class_xrcamera3d.md#class_xrcamera3d) node. Most will have two [XRController3D](../../classes/class_xrcontroller3d.md#class_xrcontroller3d), one for the left hand and one for the right. Keep in mind that the camera and controller nodes should be children of the origin node. Add these nodes to a new scene and rename the controller nodes to `LeftHand` and `RightHand`, your scene should look something like this:

![](img/xr_basic_scene.webp)
...
![](img/xr_left_hand.webp)

And the right hand:

![](img/xr_right_hand.webp)
...
If you run your project at this point in time, everything will work but you will be in a dark world. So to finish off our starting point add a [DirectionalLight3D](../../classes/class_directionallight3d.md#class_directionallight3d) and a [WorldEnvironment](../../classes/class_worldenvironment.md#class_worldenvironment) node to your scene.
