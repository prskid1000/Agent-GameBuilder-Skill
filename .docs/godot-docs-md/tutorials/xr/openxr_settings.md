<span id="doc_openxr_settings"></span>
...
For other backends you can enable OpenXR at any time by calling `initialize` on the [OpenXRInterface](../../classes/class_openxrinterface.md#class_openxrinterface).
...
where we want to place the [XROrigin3D](../../classes/class_xrorigin3d.md#class_xrorigin3d) node where the player's head should be.

When the user enacts the recenter option on their headset, the method of which is different per XR runtime,
the XR runtime will move the [XRCamera3D](../../classes/class_xrcamera3d.md#class_xrcamera3d) to the [XROrigin3D](../../classes/class_xrorigin3d.md#class_xrorigin3d) node.
The [OpenXRInterface](../../classes/class_openxrinterface.md#class_openxrinterface) will also emit the `pose_recentered` signal
...
See [Room Scale](xr_room_scale.md#doc_xr_room_scale) for more information.

When the user enacts the recenter option on their headset, the method of which is different per XR runtime,
the XR runtime will not change the origin point.
The [OpenXRInterface](../../classes/class_openxrinterface.md#class_openxrinterface) will emit the `pose_recentered` signal
and it is up to the game to react appropriately.
Not doing so will prevent your game from being accepted on various stores.

In Godot you can do this by calling the `center_on_hmd` function on the [XRServer](../../classes/class_xrserver.md#class_xrserver):

- Calling `XRServer.center_on_hmd(XRServer.RESET_BUT_KEEP_TILT, false)` will move the [XRCamera3D](../../classes/class_xrcamera3d.md#class_xrcamera3d) node
  to the [XROrigin3D](../../classes/class_xrorigin3d.md#class_xrorigin3d) node similar to the `Local` reference space.
- Calling `XRServer.center_on_hmd(XRServer.RESET_BUT_KEEP_TILT, true)` will move the [XRCamera3D](../../classes/class_xrcamera3d.md#class_xrcamera3d) node
  above the [XROrigin3D](../../classes/class_xrorigin3d.md#class_xrorigin3d) node keeping the player's height, similar to the `Local Floor` reference space.
...
When the user enacts the recenter option on their headset, the method of which is different per XR runtime,
the XR runtime will move the [XRCamera3D](../../classes/class_xrcamera3d.md#class_xrcamera3d) above the [XROrigin3D](../../classes/class_xrorigin3d.md#class_xrorigin3d) node
...
<span id="doc_openxr_settings_foveation_level"></span>
...
.. Note::
  **Compatibility renderer only**,
  for Mobile and Forward+ renderer, set the `vrs_mode` property on [Viewport](../../classes/class_viewport.md#class_viewport) to `VRS_XR`.
...
See the page on [hand tracking](openxr_hand_tracking.md#doc_openxr_hand_tracking) for additional details.
...
information on how it works on the [spatial entities page](openxr_spatial_entities.md#doc_openxr_spatial_entities).
...
![](img/openxr_eye_gaze_interaction.webp)

Don't forget to save!

Next add a new [XRController3D](../../classes/class_xrcontroller3d.md#class_xrcontroller3d) node to your origin node
...
detailed guide on how to use it [here](openxr_render_models.md#doc_openxr_render_models).
...
and set them up on the XR action map page [here](xr_action_map.md#doc_binding_modifiers).
