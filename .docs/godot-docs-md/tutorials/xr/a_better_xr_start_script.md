<span id="doc_a_better_xr_start_script"></span>

## A better XR start script

In [doc_setting_up_xr](setting_up_xr.md#doc_setting_up_xr) we introduced a startup script that initialises our setup which we used as our script on our main node.
...
Alternatively, if you are using XR Tools (see [doc_introducing_xr_tools](introducing_xr_tools.md#doc_introducing_xr_tools)) it contains a version of this script updated with some features related to XR tools.
...
- `xr_interface` holds a reference to our XR interface, this already existed but we now type it to get full access to our [XRInterface](../../classes/class_xrinterface.md#class_xrinterface) API.
...
See [OpenXR Settings](openxr_settings.md#doc_openxr_settings) for further details.

We hook up a number of signals that will be emitted by the [XRInterface](../../classes/class_xrinterface.md#class_xrinterface).
...
Often it is enough to call [center_on_hmd()](../../classes/class_xrserver.md#class_XRServer_method_center_on_hmd).
