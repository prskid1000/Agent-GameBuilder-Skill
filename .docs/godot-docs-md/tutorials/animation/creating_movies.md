<span id="doc_creating_movies"></span>
...
- Recording cutscenes that will be [displayed as pre-recorded videos](playing_videos.md#doc_playing_videos)
...
- Render at a higher framerate than the video's target framerate, then
  [post-process to generate high-quality motion blur](creating_movies.md#doc_creating_movies_motion_blur).
...
> **Warning:** **This feature is not designed for capturing real-time footage during gameplay.** Players should use something like `OBS Studio <https://obsproject.com/>`__ or `SimpleScreenRecorder <https://www.maartenbaert.be/simplescreenrecorder/>`__ to record gameplay videos, as they do a much better job at intercepting the compositor than Godot can do using Vulkan or OpenGL natively. That said, if your game runs at near-real-time speeds when capturing, you can still use this feature (but it will lack audible sound playback, as sound is saved directly to the video file).
...
Movie Maker can also be enabled from the [command line](../editor/command_line_tutorial.md#doc_command_line_tutorial):
...
`disabled` or `canvas_items` [stretch mode](../rendering/multiple_resolutions.md#doc_multiple_resolutions):
...
[doc_creating_movies_recording_at_higher_resolution](creating_movies.md#doc_creating_movies_recording_at_higher_resolution) if you need to record
...
Output formats are provided by the [MovieWriter](../../classes/class_moviewriter.md#class_MovieWriter) class.
Godot has 3 built-in [MovieWriters](../../classes/class_moviewriter.md#class_MovieWriter), and more can be
...
> **Note:** 
   ...
   On the other hand, [OGV playback](playing_videos.md#doc_playing_videos)
...
to be
[encoded to a video file with an external tool after recording](creating_movies.md#doc_creating_movies_converting_avi).
...
[Using a video converter program](creating_movies.md#doc_creating_movies_converting_avi)
...
The `movie` [feature tag](../export/feature_tags.md#doc_feature_tags) can be used to override
...
<span id="doc_creating_movies_recording_at_higher_resolution"></span>
...
By default, Godot uses the `disabled` [stretch modes](../rendering/multiple_resolutions.md#doc_multiple_resolutions)
...
those settings with the `movie` [feature tag](../export/feature_tags.md#doc_feature_tags).
...
<span id="doc_creating_movies_converting_avi"></span>
...
<span id="doc_creating_movies_converting_image_sequence"></span>
...
<span id="doc_creating_movies_motion_blur"></span>
...
<span id="doc_creating_movies_reducing_framerate"></span>
...
See [doc_creating_movies_reducing_framerate](creating_movies.md#doc_creating_movies_reducing_framerate) if you want to get this benefit
