<span id="doc_matrices_and_transforms"></span>
...
and understand the [doc_vector_math](vector_math.md#doc_vector_math) tutorial, as this tutorial
...
Most of this guide focuses on 2D, using [class_Transform2D](../../classes/class_transform2d.md#class_Transform2D) and
[class_Vector2](../../classes/class_vector2.md#class_Vector2), but the way things work in 3D is very similar.

> **Note:** remember that in Godot, the Y axis points *down* in 2D. This is the opposite of how most schools teach linear algebra, with the Y axis pointing up.
> **Note:** green, and the Z axis is blue. This tutorial is color-coded to match these conventions, but we will also represent the origin vector with a blue color.
...
> **Note:** method to perform scaling.
...
> **Note:** A full turn is `TAU` or `PI*2` radians, and a quarter turn of 90 degrees is `TAU/4` or `PI/2` radians. Working with `TAU` usually results in more readable code.
> **Note:** is represented clockwise. This means that all the math and trig functions behave the same as a Y-is-up CCW system, since these differences "cancel out". You can think of rotations in both systems being "from X to Y".
...
> **Note:** method to perform rotations.
...
You might have noticed that [class_Transform2D](../../classes/class_transform2d.md#class_Transform2D) actually
has three [class_Vector2](../../classes/class_vector2.md#class_Vector2) values: `x`, `y`, and `origin`.
...
Note that in 3D, Godot has a separate [class_Basis](../../classes/class_basis.md#class_Basis) structure
for holding the three [class_Vector3](../../classes/class_vector3.md#class_Vector3) values of the basis,
since the code can get complex and it makes sense to separate
it from [class_Transform3D](../../classes/class_transform3d.md#class_Transform3D) (which is composed of one
[class_Basis](../../classes/class_basis.md#class_Basis) and one extra [class_Vector3](../../classes/class_vector3.md#class_Vector3) for the origin).
...
> **Note:** projects you will want to translate by hundreds of units.
...
> **Note:** feel free to skip this section of the tutorial. This section explores an uncommonly used aspect of transformation matrices for the purpose of building an understanding of them. Node2D provides a shearing property out of the box.
...
matrix has four total numbers in two [class_Vector2](../../classes/class_vector2.md#class_Vector2) values, while
...
> **Note:** so you *must* use code if you want to shear the object.
...
> **Note:** When we refer to "world position", that would be relative to the node's parent instead, if the node had a parent.
...
transforms by having multiple [class_Node2D](../../classes/class_node2d.md#class_Node2D) or [class_Node3D](../../classes/class_node3d.md#class_Node3D)
nodes parented to each other.

However, it's useful to understand how to manually calculate the values we
need. We will go over how you could use [class_Transform2D](../../classes/class_transform2d.md#class_Transform2D) or
[class_Transform3D](../../classes/class_transform3d.md#class_Transform3D) to manually calculate transforms of nodes.
...
> **Note:** (0, 0), you can use the "basis_xform" or "basis_xform_inv" methods instead, which skip dealing with translation.
...
> **Note:** or ``move_local_x`` and ``move_local_y`` in 2D to do this.
...
axis is of type [class_Vector3](../../classes/class_vector3.md#class_Vector3), and also that Godot stores
the [class_Basis](../../classes/class_basis.md#class_Basis) separately from the [class_Transform3D](../../classes/class_transform3d.md#class_Transform3D),
...
[class_Basis](../../classes/class_basis.md#class_Basis) vectors and the origin in both 2D and 3D:
...
> **Note:** inspector. This may be changed in a future release of Godot.
If you would like additional explanation, you should check out
...
represent quaternions using the [class_Quaternion](../../classes/class_quaternion.md#class_Quaternion) struct. My suggestion
