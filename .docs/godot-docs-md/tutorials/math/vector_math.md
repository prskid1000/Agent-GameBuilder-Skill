...
> **Note:** only be looking at how it is applied to game development. For a broader look at the mathematics, see https://www.khanacademy.org/math/linear-algebra
...
> **Note:** positive ``y`` axis points **downwards** instead of upwards, as you probably learned in math class. However, this is common in most computer graphics applications.
...
Godot supports both [Vector2](../../classes/class_vector2.md#class_Vector2) and :ref:`Vector3
...
> **Note:** only magnitude is called a **scalar**. Scalars use the :ref:`class_float` type in Godot.
...
> **Note:** its magnitude. Multiplying with a negative scalar results in a vector in the opposite direction. This is how you **scale** a vector.
...
.. tip:: Velocity measures the **change** in position per unit of time. The new
         ...
         the previous frame) from the [_process()](../../classes/class_node.md#class_Node_private_method__process)
         or [_physics_process()](../../classes/class_node.md#class_Node_private_method__physics_process)
         callbacks.
...
[normalized()](../../classes/class_vector2.md#class_Vector2_method_normalized) method for this:
...
> **Warning:** cannot normalize a vector of length ``0``. Attempting to do so would normally result in an error. In GDScript though, trying to call the ``normalized()`` method on a vector of length 0 leaves the value untouched and avoids the error for you.
### Reflection
...
is a [bounce()](../../classes/class_vector2.md#class_Vector2_method_bounce) method to handle this.
...
called [direction_to()](../../classes/class_vector2.md#class_Vector2_method_direction_to). If the angle
...
With Godot, you can use the built-in [Vector3.cross()](../../classes/class_vector3.md#class_Vector3_method_cross)
method:
...
> **Note:** same result as ``b.cross(a)``. The resulting vectors point in **opposite** directions.
### Calculating normals
...
- [doc_vectors_advanced](vectors_advanced.md#doc_vectors_advanced)
- [doc_matrices_and_transforms](matrices_and_transforms.md#doc_matrices_and_transforms)
