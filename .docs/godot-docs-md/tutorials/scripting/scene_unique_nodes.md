<span id="doc_scene_unique_nodes"></span>
...
![](img/unique_name_scene_instance_example.webp)
...
A [Group](groups.md#doc_groups) allows locating a node (or a group of many nodes)
from any other node, no matter what scene the two nodes are located in.

A [Singleton (Autoload)](singletons_autoload.md#doc_singletons_autoload) is an always loaded node
...
[Node.find_child()](../../classes/class_node.md#class_Node_method_find_child) finds a node by name
