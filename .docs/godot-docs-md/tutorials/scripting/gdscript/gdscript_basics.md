<span id="doc_gdscript_reference"></span>

## GDScript reference

[GDScript](index.md#doc_gdscript) is a high-level, `object-oriented
...
> **Note:** Documentation about GDScript's history has been moved to the :ref:`Frequently Asked Questions <doc_faq_what_is_gdscript>`.
### Example of GDScript
...
read this tutorial: [doc_gdscript_more_efficiently](gdscript_advanced.md#doc_gdscript_more_efficiently).
...
|                                       | [format strings](gdscript_format_string.md#doc_gdscript_printf).                                |
|                                       |                                                                             |
...
+---------------------------------------+-----------------------------------------------------------------------------+
| | `not x`                           | Boolean NOT and its [unrecommended](gdscript_styleguide.md#boolean_operators) alias          |
| | `!x`                              |                                                                             |
+---------------------------------------+-----------------------------------------------------------------------------+
| | `x and y`                         | Boolean AND and its [unrecommended](gdscript_styleguide.md#boolean_operators) alias          |
| | `x && y`                          |                                                                             |
+---------------------------------------+-----------------------------------------------------------------------------+
| | `x or y`                          | Boolean OR and its [unrecommended](gdscript_styleguide.md#boolean_operators) alias           |
| | `x || y`                          |                                                                             |
+---------------------------------------+-----------------------------------------------------------------------------+
...
> **Note:** The behavior of some operators may differ from what you expect: 1. If both operands of the ``/`` operator are :ref:`int <class_int>`, then integer division is performed instead of fractional. For example ``5 / 2 == 2``, not ``2.5``. If this is not desired, use at least one :ref:`float <class_float>` literal (``x / 2.0``), cast (``float(x) / y``), or multiply by ``1.0`` (``x * 1.0 / y``). 2. The ``%`` operator is only available for ints, for floats use the :ref:`fmod() <class_@GlobalScope_method_fmod>` function. 3. For negative values, the ``%`` operator and ``fmod()`` use `truncation <https://en.wikipedia.org/wiki/Truncation>`_ instead of rounding towards negative infinity. This means that the remainder has a sign. If you need the remainder in a mathematical sense, use the :ref:`posmod() <class_@GlobalScope_method_posmod>` and :ref:`fposmod() <class_@GlobalScope_method_fposmod>` functions instead. 4. The ``==`` and ``!=`` operators sometimes allow you to compare values of different types (for example, ``1 == 1.0`` is true), but in other cases it can cause a runtime error. If you're not sure about the types of the operands, you can safely use the :ref:`is_same() <class_@GlobalScope_method_is_same>` function (but note that it is more strict about types and references). To compare floats, use the :ref:`is_equal_approx() <class_@GlobalScope_method_is_equal_approx>` and :ref:`is_zero_approx() <class_@GlobalScope_method_is_zero_approx>` functions instead.
### Literals
...
+---------------------------------+-------------------------------------------+
| `&"name"`                     | [StringName](../../../classes/class_stringname.md#class_StringName)      |
+---------------------------------+-------------------------------------------+
| `^"Node/Label"`               | [NodePath](../../../classes/class_nodepath.md#class_NodePath)          |
+---------------------------------+-------------------------------------------+
...
GDScript also supports [format strings](gdscript_format_string.md#doc_gdscript_printf).
...
For more information about exporting properties, read the [GDScript exports](gdscript_exports.md#doc_gdscript_exports)
article.
...
<span id="doc_gdscript_onready_annotation"></span>
...
> **Warning:** Applying ``@onready`` and any ``@export`` annotation to the same variable doesn't work as you might expect. The ``@onready`` annotation will cause the default value to be set after the ``@export`` takes effect and will override it: :: @export var a = "init_value_a" @onready @export var b = "init_value_b" func _init(): prints(a, b) # init_value_a <null> func _notification(what): if what == NOTIFICATION_SCENE_INSTANTIATED: prints(a, b) # exported_value_a exported_value_b func _ready(): prints(a, b) # exported_value_a init_value_b Therefore, the ``ONREADY_WITH_EXPORT`` warning is generated, which is treated as an error by default. We do not recommend disabling or ignoring it.
### Comments
...
[doc_gdscript_documentation_comments](gdscript_documentation_comments.md#doc_gdscript_documentation_comments) for details.
...
<span id="doc_gdscript_builtin_types"></span>
...
[Variant types](../../../engine_details/architecture/variant_class.md#doc_variant_class) must have a valid value at all times,
...
the tree to get a node, or affecting properties like with [Tweens](../../../classes/class_tween.md#class_Tween).
...
> **Note:** Arrays are passed by reference, so the array element type is also an attribute of the in-memory structure referenced by a variable in runtime. The static type of a variable restricts the structures that it can reference to. Therefore, you **cannot** assign an array with a different element type, even if the type is a subtype of the required type. If you want to *convert* a typed array, you can create a new array and use the :ref:`Array.assign() <class_Array_method_assign>` method: :: var a: Array[Node2D] = [Node2D.new()] # (OK) You can add the value to the array because `Node2D` extends `Node`. var b: Array[Node] = [a[0]] # (Error) You cannot assign an `Array[Node2D]` to an `Array[Node]` variable. b = a # (OK) But you can use the `assign()` method instead. Unlike the `=` operator, # the `assign()` method copies the contents of the array, not the reference. b.assign(a) The only exception was made for the ``Array`` (``Array[Variant]``) type, for user convenience and compatibility with old code. However, operations on untyped arrays are considered unsafe.
<span id="doc_gdscript_packed_arrays"></span>
...
methods such as [Array.map](../../../classes/class_array.md#class_Array_method_map) that PackedArrays
lack. Consult the [class reference](../../../classes/class_packedfloat32array.md#class_PackedFloat32Array) for details
...
- [PackedByteArray](../../../classes/class_packedbytearray.md#class_PackedByteArray): An array of bytes (integers from 0 to 255).
- [PackedInt32Array](../../../classes/class_packedint32array.md#class_PackedInt32Array): An array of 32-bit integers.
- [PackedInt64Array](../../../classes/class_packedint64array.md#class_PackedInt64Array): An array of 64-bit integers.
- [PackedFloat32Array](../../../classes/class_packedfloat32array.md#class_PackedFloat32Array): An array of 32-bit floats.
- [PackedFloat64Array](../../../classes/class_packedfloat64array.md#class_PackedFloat64Array): An array of 64-bit floats.
- [PackedStringArray](../../../classes/class_packedstringarray.md#class_PackedStringArray): An array of strings.
- [PackedVector2Array](../../../classes/class_packedvector2array.md#class_PackedVector2Array): An array of [Vector2](../../../classes/class_vector2.md#class_Vector2) values.
- [PackedVector3Array](../../../classes/class_packedvector3array.md#class_PackedVector3Array): An array of [Vector3](../../../classes/class_vector3.md#class_Vector3) values.
- [PackedVector4Array](../../../classes/class_packedvector4array.md#class_PackedVector4Array): An array of [Vector4](../../../classes/class_vector4.md#class_Vector4) values.
- [PackedColorArray](../../../classes/class_packedcolorarray.md#class_PackedColorArray): An array of [Color](../../../classes/class_color.md#class_Color) values.
...
> **Note:** The bracket syntax can be used to access properties of any :ref:`class_Object`, not just Dictionaries. Keep in mind it will cause a script error when attempting to index a non-existing property. To avoid this, use the :ref:`Object.get() <class_Object_method_get>` and :ref:`Object.set() <class_Object_method_set>` methods instead.
#### Typed dictionaries
...
> **Note:** When referencing a static variable from a tool script, the other script containing the static variable **must** also be a tool script. See :ref:`Running code in the editor <doc_running_code_in_the_editor_important_information>` for details.
...
[Dictionary](../../../classes/class_dictionary.md#class_Dictionary) of that name. This means all constant methods of
...
> **Note:** branching statements (such as an ``if``/``else`` construct), all the possible paths must have a return. E.g., if you have a ``return`` inside an ``if`` block but not after it, the editor will raise an error because if the block is not executed, the function won't have a valid value to return.
Referencing functions
~~~~~~~~~~~~~~~~~~~~~

Functions are first-class values in terms of the [Callable](../../../classes/class_callable.md#class_Callable) object.
...
> **Note:** Callables **must** be called with the :ref:`call() <class_Callable_method_call>` method. You cannot use the ``()`` operator directly. This behavior is implemented to avoid performance issues on direct function calls.
Lambda functions
~~~~~~~~~~~~~~~~

Lambda functions allow you to declare functions that do not belong to a class. Instead, a
[Callable](../../../classes/class_callable.md#class_Callable) object is created and assigned to a variable directly.
...
To call the created lambda you can use the [call()](../../../classes/class_callable.md#class_Callable_method_call) method:
...
> **Note:** Although you can declare functions as variadic using the rest parameter, unpacking parameters when calling a function using *spread syntax* that exists in some languages ​​(JavaScript, PHP) is currently not supported in GDScript. However, you can use ``callv()`` to call a function with an array of arguments: :: func test_func(...args): #log_data(...args) # This won't work. log_data.callv(args) # This will work. func log_data(...values): # You should use `callv()` if you want to pass `values` as the argument list, # rather than passing the array as the first argument. prints.callv(values) # You can use array concatenation to prepend/append the argument list. write_data.callv(["user://log.txt"] + values) func write_data(path, ...values): # ...
Abstract functions
~~~~~~~~~~~~~~~~~~
...
provided using [_set()](../../../classes/class_object.md#class_Object_private_method__set) and/or
[_get()](../../../classes/class_object.md#class_Object_private_method__get)).
...
<span id="doc_gdscript_basics_class_name"></span>
...
.. tip::
    ...
    **Editor > Scale With Editor Scale** and **Editor > Convert Icons With Editor Theme**
    [import options](../../assets_pipeline/importing_images.md#doc_importing_images_editor_import_options) enabled. This allows
...
<span id="doc_gdscript_basics_abstract_class"></span>
...
> **Warning:** Since an abstract class cannot be instantiated, it is not possible to attach an abstract class to a node. If you attempt to do so, the engine will print an error when running the scene: .. code-block:: none Cannot set object script. Script '<path to script>' should not be abstract.
...
> **Note:** If inheritance is not explicitly defined, the class will default to inheriting :ref:`class_RefCounted`.
...
<span id="doc_gdscript_basics_inner_classes"></span>
...
<span id="doc_gdscript_classes_as_resources"></span>

Classes as resources
~~~~~~~~~~~~~~~~~~~~

Classes stored as files are treated as [GDScripts](../../../classes/class_gdscript.md#class_GDScript). They
...
> **Note:** Documentation about exports has been moved to :ref:`doc_gdscript_exports`.
<span id="doc_gdscript_basics_setters_getters"></span>
...
<span id="doc_gdscript_tool_mode"></span>
...
See [doc_running_code_in_the_editor](../../plugins/running_code_in_the_editor.md#doc_running_code_in_the_editor) for more information.

> **Warning:** in a tool script (especially the script's owner itself). As tool scripts run their code in the editor, misusing them may lead to crashing the editor.
<span id="doc_gdscript_basics_memory_management"></span>
...
Any instance of the [class_RefCounted](../../../classes/class_refcounted.md#class_RefCounted) class (or any class that inherits
it, such as [class_Resource](../../../classes/class_resource.md#class_Resource)) will be freed automatically when no longer
in use. For an instance of any class that is not a [class_RefCounted](../../../classes/class_refcounted.md#class_RefCounted)
(such as [class_Node](../../../classes/class_node.md#class_Node) or the base [class_Object](../../../classes/class_object.md#class_Object) type), it will
remain in memory until it is deleted with `free()` (or `queue_free()`
for Nodes).

> **Note:** If a :ref:`class_Node` is deleted via ``free()`` or ``queue_free()``, all of its children will also recursively be deleted.
To avoid reference cycles that can't be freed, a [class_WeakRef](../../../classes/class_weakref.md#class_WeakRef)
function is provided for creating weak references, which allow access
to the object without preventing a [class_RefCounted](../../../classes/class_refcounted.md#class_RefCounted) from freeing.
Here is an example:
...
<span id="doc_gdscript_signals"></span>
...
signals of nodes like [class_Button](../../../classes/class_button.md#class_Button) or [class_RigidBody3D](../../../classes/class_rigidbody3d.md#class_RigidBody3D).
...
it with [Signal.emit()](../../../classes/class_signal.md#class_Signal_method_emit), and from
a `Game` node higher up our scene tree, we connect it to the `Lifebar` using
the [Signal.connect()](../../../classes/class_signal.md#class_Signal_method_connect) method:
...
arguments using [Callable.bind()](../../../classes/class_callable.md#class_Callable_method_bind). This
...
> **Note:** This is done to ensure type safety. With this type safety in place, a function cannot say that it returns an ``int`` while it actually returns a function state object during runtime.
### Assert keyword
