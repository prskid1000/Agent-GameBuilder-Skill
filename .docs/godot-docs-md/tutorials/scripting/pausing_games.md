<span id="doc_pausing_games"></span>
...
`true` to the [SceneTree.paused](../../classes/class_scenetree.md#class_SceneTree_property_paused) property:
...
> **Note:** paused by using their ``set_active`` methods.
### Process Modes

Each node in Godot has a "Process Mode" that defines when it processes. It can
be found and changed under a node's [Node](../../classes/class_node.md#class_Node) properties in the inspector.
