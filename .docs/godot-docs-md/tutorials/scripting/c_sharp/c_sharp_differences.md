<span id="doc_c_sharp_differences"></span>
...
As explained in [doc_c_sharp_general_differences](c_sharp_basics.md#doc_c_sharp_general_differences), `PascalCase` is used
...
reasonably possible. See the [doc_c_sharp_styleguide](c_sharp_style_guide.md#doc_c_sharp_styleguide), which we encourage
...
are considered essential. See [doc_c_sharp_setup_external_editor](c_sharp_basics.md#doc_c_sharp_setup_external_editor).
...
See also: [doc_c_sharp_exports](c_sharp_exports.md#doc_c_sharp_exports).
...
See also: [doc_c_sharp_signals](c_sharp_signals.md#doc_c_sharp_signals).

### `@onready` annotation

GDScript has the ability to defer the initialization of a member variable until the ready function
is called with `@onready` (cf. [doc_gdscript_onready_annotation](../gdscript/gdscript_basics.md#doc_gdscript_onready_annotation)).
For example:
...
> **Note:** .NET provides path utility methods under the `System.IO.Path`_ class. They can only be used with native OS paths, not Godot paths (paths that start with ``res://`` or ``user://``). See :ref:`doc_data_paths`.
...
the `await` keyword. See [doc_c_sharp_differences_await](c_sharp_differences.md#doc_c_sharp_differences_await).

Instead of using the `Signal` type, the recommended way to use Godot signals in C# is
to use the generated C# events. See [doc_c_sharp_signals](c_sharp_signals.md#doc_c_sharp_signals).
...
See also [PackedArray in C#](c_sharp_collections.md#doc_c_sharp_collections_packedarray).
...
See also [Array in C#](c_sharp_collections.md#doc_c_sharp_collections_array).
...
See also [Dictionary in C#](c_sharp_collections.md#doc_c_sharp_collections_dictionary).

### Variant

`Godot.Variant` is used to represent Godot's native [Variant](../../../classes/class_variant.md#class_Variant) type.
Any [Variant-compatible type](c_sharp_variant.md#c_sharp_variant_compatible_types) can be converted from/to it.

See also: [doc_c_sharp_variant](c_sharp_variant.md#doc_c_sharp_variant).

### Communicating with other scripting languages

This is explained extensively in [doc_cross_language_scripting](../cross_language_scripting.md#doc_cross_language_scripting).

<span id="doc_c_sharp_differences_await"></span>
