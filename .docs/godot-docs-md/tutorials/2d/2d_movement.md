...
We'll use [CharacterBody2D](../../classes/class_characterbody2d.md#class_CharacterBody2D) for these examples,
but the principles will apply to other node types (Area2D, RigidBody2D) as well.

<span id="doc_2d_movement_setup"></span>
...
input actions (see [InputEvent](../inputs/inputevent.md#doc_inputevent) for details):

![](img/movement_inputs.webp)
...
In the `get_input()` function, we use [Input](../../classes/class_input.md#class_Input) `get_vector()` to check for the
...
.. tip:: If you've never used vector math before, or need a refresher,
         you can see an explanation of vector usage in Godot at [doc_vector_math](../math/vector_math.md#doc_vector_math).

> **Note:** If the code above does nothing when you press the keys, double-check that you've set up input actions correctly as described in the :ref:`doc_2d_movement_setup` part of this tutorial.
### Rotation + movement
...
Here we're using the [Node2D](../../classes/class_node2d.md#class_Node2D) `look_at()` method to
