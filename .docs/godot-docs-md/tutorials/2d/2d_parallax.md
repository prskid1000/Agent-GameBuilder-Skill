...
provides the [Parallax2D](../../classes/class_parallax2d.md#class_parallax2d) node to achieve this effect. It can still be easy to get tripped
up though, so this page provides in-depth descriptions of some properties and how to fix some common mistakes.

> **Note:** This page covers how to use :ref:`Parallax2D<class_parallax2d>`, which is recommended to use over the :ref:`ParallaxLayer<class_parallaxlayer>` and :ref:`ParallaxBackground<class_parallaxbackground>` nodes.
### Getting started
...
on [positioning](2d_parallax.md#doc_2d_parallax_positioning) for why this is important.

![](img/2d_parallax_size_viewport.webp)

The scene above uses one prepared texture for the higher clouds in a [Sprite2D](../../classes/class_sprite2d.md#class_sprite2d), but you could
just as easily use multiple nodes spaced out to compose the layer.

### Scroll scale

The backbone of the parallax effect is the [scroll_scale](../../classes/class_parallax2d.md#class_parallax2d_property_scroll_scale) property.
...
The scene above is comprised of five layers. Some good [scroll_scale](../../classes/class_parallax2d.md#class_parallax2d_property_scroll_scale)
values might be:
...
[Parallax2D](../../classes/class_parallax2d.md#class_parallax2d) provides a bonus effect that gives textures the illusion of repeating infinitely.
[repeat_size](../../classes/class_parallax2d.md#class_parallax2d_property_repeat_size) tells the node to snap its position forward or back when the
...
size or larger than your viewport **before** setting the [repeat_size](../../classes/class_parallax2d.md#class_parallax2d_property_repeat_size). If
...
![](img/2d_parallax_size_bad.webp)

We can see that the viewport size is 500x300 but the texture is 288x208. If we set the
[repeat_size](../../classes/class_parallax2d.md#class_parallax2d_property_repeat_size) to the size of our image, the infinite repeat effect doesn't
scroll properly because the original texture doesn't cover the viewport. If we set the
[repeat_size](../../classes/class_parallax2d.md#class_parallax2d_property_repeat_size) to the size of the viewport, we have a large gap. What can we
do?
...
larger to fit your screen. Set the [scale](../../classes/class_node2d.md#class_node2d_property_scale) of the [Parallax2D](../../classes/class_parallax2d.md#class_parallax2d),
and all child textures scale with it.

#### Scale the child nodes

Similar to scaling the [Parallax2D](../../classes/class_parallax2d.md#class_parallax2d), you can scale your [Sprite2D](../../classes/class_sprite2d.md#class_sprite2d) nodes to
be large enough to cover the screen. Keep in mind that some settings like
[Parallax2D.repeat_size](../../classes/class_parallax2d.md#class_parallax2d_property_repeat_size) and
[Sprite2D.region_rect](../../classes/class_sprite2d.md#class_sprite2d_property_region_rect) do not take scaling into account, so it's necessary to
adjust these values based on the scale.

![](img/2d_parallax_size_scale.webp)

#### Repeat the textures

You can also start off on the right foot by preparing child nodes earlier in the process. If you have a
[Sprite2D](../../classes/class_sprite2d.md#class_sprite2d) you'd like to repeat, but is too small, you can do the following to repeat it:

- set [texture_repeat](../../classes/class_canvasitem.md#class_canvasitem_property_texture_repeat) to [CanvasItem.TEXTURE_REPEAT_ENABLED](../../classes/class_canvasitem.md#class_canvasitem_constant_TEXTURE_REPEAT_ENABLED)
- set [region_enabled](../../classes/class_sprite2d.md#class_sprite2d_property_region_enabled) to `true`
- set the [region_rect](../../classes/class_sprite2d.md#class_sprite2d_property_region_rect) to a multiple of the size of your texture large enough to cover the viewport.

Below, you can see that repeating the image twice makes it large enough to cover the screen.

![](img/2d_parallax_size_repeat.webp)

<span id="doc_2d_parallax_positioning"></span>
...
![](img/2d_parallax_single_centered.webp)

This creates problems with the infinite repeat effect and should be avoided. The "infinite repeat canvas" starts at
`(0,0)` and expands down and to the right to the size of the [repeat_size](../../classes/class_parallax2d.md#class_parallax2d_property_repeat_size)
value.

![](img/2d_parallax_single_expand.webp)
...
Increasing [repeat_times](../../classes/class_parallax2d.md#class_parallax2d_property_repeat_times) technically *would* work in some scenarios, but
...
[Parallax2D.repeat_size](../../classes/class_parallax2d.md#class_parallax2d_property_repeat_size) is set correctly, it should look something like
this, with one single loop of the image the same size or larger than the viewport:

![](img/2d_parallax_repeat_good_norect.webp)

If you think of how the image scrolls across the screen, it starts by displaying what's inside the red rectangle
(determined by [repeat_size](../../classes/class_parallax2d.md#class_parallax2d_property_repeat_size)), and when it reaches what's inside the yellow
rectangle it zips the image forward to give the illusion of scrolling forever.

![](img/2d_parallax_repeat_good.webp)
...
![](img/2d_parallax_repeat_bad.webp)

### Scroll offset

If your parallax textures are already working correctly, but you prefer it to start at a different point,
[Parallax2D](../../classes/class_parallax2d.md#class_parallax2d) comes with a [scroll_offset](../../classes/class_parallax2d.md#class_parallax2d_property_scroll_offset) property
used to offset where the infinite repeat canvas starts. As an example, if your image is 288x208, setting
the [scroll_offset](../../classes/class_parallax2d.md#class_parallax2d_property_scroll_offset) to `(-144,0)` or `(144,0)` allows it to begin
...
occur when we zoom out by setting the [Camera2D.zoom](../../classes/class_camera2d.md#class_camera2d_property_zoom) to `(0.5, 0.5)`:

![](img/2d_parallax_zoom_single.webp)
...
[repeat_times](../../classes/class_parallax2d.md#class_parallax2d_property_repeat_times) can help out. Setting a value of `3` (one extra
repeat behind and in front), it is now large enough to accommodate the infinite repeat effect.

![](img/2d_parallax_zoom_repeat_times.webp)

If these textures were meant to be repeated vertically, we would have specified a `y` value for the
[repeat_size](../../classes/class_parallax2d.md#class_parallax2d_property_repeat_size). The
[repeat_times](../../classes/class_parallax2d.md#class_parallax2d_property_repeat_times) would automatically add a repeat above and below as well.
...
![](img/2d_parallax_zoom_repeat_adjusted.webp)

### Split screen

Most tutorials for making a split screen game in Godot begin by writing a small script to assign
the [Viewport.world_2d](../../classes/class_viewport.md#class_viewport_property_world_2d) of the first SubViewport to the second, so they have a
...
[SubViewport](../../classes/class_subviewport.md#class_subviewport). Here's how a setup looks for a two player game:

![](img/2d_parallax_splitscreen.webp)
...
- Leave all parallax nodes at their default [visibility_layer](../../classes/class_canvasitem.md#class_canvasitem_property_visibility_layer) of 1.
- Set the first SubViewport's [canvas_cull_mask](../../classes/class_viewport.md#class_viewport_property_canvas_cull_mask) to only layers 1 and 2.
- Do the same for the second SubViewport but use layers 1 and 3.
- Give your parallax nodes in the first SubViewport a common parent and set its [visibility_layer](../../classes/class_canvasitem.md#class_canvasitem_property_visibility_layer) to 2.
- Do the same for the second SubViewport's parallax nodes, but use a layer of 3.

How does this work? If a canvas item has a [visibility_layer](../../classes/class_canvasitem.md#class_canvasitem_property_visibility_layer) that
doesn't match the SubViewport's [canvas_cull_mask](../../classes/class_viewport.md#class_viewport_property_canvas_cull_mask), it will hide all
children, even if they do. We use this to our advantage, letting the SubViewports cut off rendering of parallax nodes
whose parent doesn't have a supported [visibility_layer](../../classes/class_canvasitem.md#class_canvasitem_property_visibility_layer).

### Previewing in the editor

Prior to 4.3, the recommendation was to place every layer in their own
[ParallaxBackground](../../classes/class_parallaxbackground.md#class_parallaxbackground), enable the
[follow_viewport_enabled](../../classes/class_canvaslayer.md#class_canvaslayer_property_follow_viewport_enabled) property, and scale the individual
layer. This method has always been tricky to get right, but is still achievable by using a
[CanvasLayer](../../classes/class_canvaslayer.md#class_canvaslayer) instead of a [ParallaxBackground](../../classes/class_parallaxbackground.md#class_parallaxbackground).

> **Note:** Another recommendation is `KoBeWi's "Parallax2D Preview" addon <https://github.com/KoBeWi/Godot-Parallax2D-Preview>`_. It provides a few different preview modes and is very handy!
