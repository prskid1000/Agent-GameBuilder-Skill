<span id="doc_compositor"></span>

## The Compositor

The compositor is a new feature in Godot 4 that allows control over
the rendering pipeline when rendering the contents of a [Viewport](../../classes/class_viewport.md#class_Viewport).

It can be configured on a [WorldEnvironment](../../classes/class_worldenvironment.md#class_WorldEnvironment)
node where it applies to all Viewports, or it can be configured on
a [Camera3D](../../classes/class_camera3d.md#class_Camera3D) and apply only to
the Viewport using that camera.

The [Compositor](../../classes/class_compositor.md#class_Compositor) resource is used to configure
the compositor. To get started, create a new compositor on the appropriate node:

![](img/new_compositor.webp)
...
We need to extend our node from [CompositorEffect](../../classes/class_compositoreffect.md#class_CompositorEffect).
...
please check [Using compute shaders](../shaders/compute_shaders.md#doc_compute_shaders).
...
Note the use of a [Mutex](../../classes/class_mutex.md#class_Mutex) in our code.
...
Note that we cast this to [RenderSceneBuffersRD](../../classes/class_renderscenebuffersrd.md#class_RenderSceneBuffersRD)
...
Note the use of our [UniformSetCacheRD](../../classes/class_uniformsetcacherd.md#class_UniformSetCacheRD) cache
...
![](img/add_compositor_effect.webp)
...
![](img/post_process_shader.webp)

> **Note:** For a more advanced example of post effects, check out the `Radial blur based sky rays <https://github.com/BastiaanOlij/RERadialSunRays>`_ example project created by Bastiaan Olij.
