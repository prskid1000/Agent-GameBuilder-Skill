<span id="doc_creating_applications"></span>
...
.. seealso::
    ...
    Check out Material Maker and
    Pixelorama for examples of
...
Additional windows can be created using the [class_Window](../../classes/class_window.md#class_Window) node.
...
minimizing the main window, setting its [unfocusable](../../classes/class_window.md#class_Window_property_unfocusable)
...
such as a [tray icon](creating_applications.md#doc_creating_applications_tray_icon).
...
Size limits can be enforced using the [min_size](../../classes/class_window.md#class_Window_property_min_size)
and [max_size](../../classes/class_window.md#class_Window_property_max_size) properties on a Window node.
Remember to multiply these size limits according to the application's scale factor
(see [doc_creating_applications_scaling_to_hidpi_displays](creating_applications.md#doc_creating_applications_scaling_to_hidpi_displays) for details).

.. tip::

    As a reminder, you can retrieve the root Window node to set properties
    on it using [get_window()](../../classes/class_node.md#class_Node_method_get_window) on any Node.

#### Using native file dialogs

*This is only supported on Windows, macOS, Linux, and Android.*

By default, Godot uses its own [class_FileDialog](../../classes/class_filedialog.md#class_FileDialog) implementation for file dialogs.
...
enabling the [use_native_dialog](../../classes/class_filedialog.md#class_FileDialog_property_use_native_dialog)
...
> **Note:** See the :ref:`property description <class_FileDialog_property_use_native_dialog>` for details on platform support. Additionally, on macOS, native file dialogs are not supported when game embedding is enabled in the editor. To test this functionality when running the project, make sure you disable game embedding by switching to the :menu:`Game` screen, clicking the 3 vertical dots and unchecking :menu:`Embed Game on Next Play`.
<span id="doc_creating_applications_tray_icon"></span>
...
notification area) using a [class_StatusIndicator](../../classes/class_statusindicator.md#class_StatusIndicator) node.
In addition to a tooltip, this node can have a [class_PopupMenu](../../classes/class_popupmenu.md#class_PopupMenu) node assigned,
so that a dropdown can be shown when clicking the icon.

StatusIndicator also has a [pressed](../../classes/class_statusindicator.md#class_StatusIndicator_signal_pressed)
...
attach this script to an [Autoload](../scripting/singletons_autoload.md#doc_singletons_autoload)
...
See [doc_handling_quit_requests](../inputs/handling_quit_requests.md#doc_handling_quit_requests) for details on overriding the behavior
...
Godot supports creating menus through the [class_MenuBar](../../classes/class_menubar.md#class_MenuBar) node, which displays
its [class_PopupMenu](../../classes/class_popupmenu.md#class_PopupMenu) children as menus. You can enable the global menu
support on a given MenuBar node by enabling its
[prefer_global_menu](../../classes/class_menubar.md#class_MenuBar_property_prefer_global_menu) property
...
> **Note:** The app menu (with the project name in bold), as well as the :menu:`Window` and :menu:`Help` menus are always present on macOS. You should not add these to the global menu manually. In Godot 4.6 and later, you can add new items within these menus by changing the :ref:`system_menu_id <class_PopupMenu_property_system_menu_id>` property on the PopupMenu node. You can choose between **Application Menu** (the first menu with the application name in bold), **Window Menu**, **Help Menu** and **Dock** (shown when right-clicking the icon in the Dock). Standard menu items that are already present in those menus will be preserved: .. figure:: img/creating_applications_native_menu_window.webp :align: center :alt: Custom options added to the system Window menu on macOS Custom options added to the system Window menu on macOS
...
For more advanced use cases, you can also use the [class_NativeMenu](../../classes/class_nativemenu.md#class_NativeMenu) singleton
directly without using the MenuBar node.

> **Note:** Global menu integration is not supported when game embedding is enabled in the editor. To test this functionality when running the project, make sure you disable game embedding by switching to the :menu:`Game` screen, clicking the 3 vertical dots and unchecking :menu:`Embed Game on Next Play`.
...
are enabled, use [DisplayServer.has_feature](../../classes/class_displayserver.md#class_DisplayServer_method_has_feature)
and also check the current value of
[Window.extend_to_title](../../classes/class_window.md#class_Window_property_extend_to_title)
...
[DisplayServer.window_get_safe_title_margins()](../../classes/class_displayserver.md#class_DisplayServer_method_window_get_safe_title_margins)
...
[DisplayServer.window_set_window_buttons_offset()](../../classes/class_displayserver.md#class_DisplayServer_method_window_set_window_buttons_offset)
...
> **Note:** On macOS, client-side decorations are not supported when game embedding is enabled in the editor. To test this functionality when running the project, make sure you disable game embedding by switching to the :menu:`Game` screen, clicking the 3 vertical dots and unchecking :menu:`Embed Game on Next Play`.
...
[Autoload](../scripting/singletons_autoload.md#doc_singletons_autoload) with this script:
...
This script can be used as an [Autoload](../scripting/singletons_autoload.md#doc_singletons_autoload)
...
set the [mouse_passthrough](../../classes/class_window.md#class_Window_property_mouse_passthrough)
property to `true` on the Window that is being drawn as an overlay.
You can also define a polygon in
[mouse_passthrough_polygon](../../classes/class_window.md#class_Window_property_mouse_passthrough_polygon),
so that certain areas can still intercept mouse input on the overlay.

Additionally, you may want to set the
[exclude_from_capture](../../classes/class_window.md#class_Window_property_exclude_from_capture)
...
> **Note:** Displaying as an overlay is not supported when game embedding is enabled in the editor. To test this functionality when running the project, make sure you disable game embedding by switching to the :menu:`Game` screen, clicking the 3 vertical dots and unchecking :menu:`Embed Game on Next Play`. Additionally, keep in mind overlays cannot be shown on top of another application if the application in question uses exclusive fullscreen. Borderless fullscreen must be used instead for overlays to be visible. There are also `known issues <https://github.com/godotengine/godot/issues/76167>`__ with transparent window display on Windows with hybrid GPU setups (such as NVIDIA Optimus). Switching renderers may help resolve the issue. On Linux with X11, transparency will not work if the user has disabled compositing in the window manager settings.
<span id="doc_creating_applications_scaling_to_hidpi_displays"></span>
...
instructions in the
[non-game application section of the Multiple resolutions documentation](../rendering/multiple_resolutions.md#doc_multiple_resolutions_non_game_application).
...
Godot uses the AccessKit library
...
using the [Control.accessibility_name](../../classes/class_control.md#class_Control_property_accessibility_name)
and [Control.accessibility_description](../../classes/class_control.md#class_Control_property_accessibility_description)
properties, and ensure the UI flows in a logical order when read by a screen reader.

.. seealso::

    See also [doc_text_to_speech](../audio/text_to_speech.md#doc_text_to_speech) for text-to-speech functionality
...
- Enable application/config/use_custom_user_dir
  ...
  [default Godot folder](../io/data_paths.md#doc_data_paths_accessing_persistent_user_data).
...
- Set physics/2d/physics_engine
  ...
  [engine compilation configuration editor](../editor/using_engine_compilation_configuration_editor.md#doc_engine_compilation_configuration_editor)
...
- Enable input_devices/pointing/android/enable_pan_and_scale_gestures
  ...
  [class_InputEventPanGesture](../../classes/class_inputeventpangesture.md#class_InputEventPanGesture) and [class_InputEventMagnifyGesture](../../classes/class_inputeventmagnifygesture.md#class_InputEventMagnifyGesture)
...
- Disable :menu:`Screen > Immersive Mode` in the Android export preset
  ...
  [DisplayServer.get_display_safe_area](../../classes/class_displayserver.md#class_DisplayServer_method_get_display_safe_area)
  and [DisplayServer.get_display_cutouts](../../classes/class_displayserver.md#class_DisplayServer_method_get_display_cutouts)
...
See [doc_optimizing_for_size](../../engine_details/development/compiling/optimizing_for_size.md#doc_optimizing_for_size) for more information on how to do this.
...
- **Windows:** Inno Setup,
  ...
  - If you have a code signing certificate, remember to sign *both*
    ...
    [sign the exported project executable](../export/exporting_for_windows.md#doc_exporting_for_windows_code_signing),
...
- [doc_runtime_loading_and_saving](../io/runtime_file_loading_and_saving.md#doc_runtime_loading_and_saving)
- [doc_http_request_class](../networking/http_request_class.md#doc_http_request_class)
- [class_ConfigFile](../../classes/class_configfile.md#class_ConfigFile) (used to save user preferences)
