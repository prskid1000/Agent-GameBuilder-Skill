<span id="doc_2d_and_3d_physics_interpolation"></span>
...
- In 3D, it is easy to turn interpolation on and off at the level of each `Node`,
  via the [physics_interpolation_mode](../../../classes/class_node.md#class_Node_property_physics_interpolation_mode)
...
> **Note:** to call ``reset_physics_interpolation()`` as with 3D.
### 2D Particles
...
- Physics interpolation in 2D is implemented on the server side, which means it's
  effective on physics bodies created using [low-level servers](../../performance/using_servers.md#doc_using_servers).
