<span id="doc_using_physics_interpolation"></span>
...
> **Note:** physics tick, the calculations for the interpolated position will be incorrect, and you will get jitter. This jitter may not be visible on your machine, but it *will* occur for some players. For this reason, setting the transform of interpolated objects should be avoided outside of the physics tick. Godot will attempt to produce warnings in the editor if this case is detected.
...
> **Note:** also by moving parent nodes in the :ref:`SceneTree<class_SceneTree>`. The movement of parents should therefore also only occur during physics ticks.
...
> **Note:** changing the project setting.
...
The solution to this is to call the [Node.reset_physics_interpolation](../../../classes/class_node.md#class_Node_method_reset_physics_interpolation)
...
cases where you should be calling [Node.reset_physics_interpolation](../../../classes/class_node.md#class_Node_method_reset_physics_interpolation),
or where you should be using your own custom interpolation on e.g. a
[Camera3D](../../../classes/class_camera3d.md#class_Camera3D). Once you have these cases fixed, you can set the
