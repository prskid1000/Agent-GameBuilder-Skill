<span id="doc_using_transforms"></span>
...
The result of all this is that you should **not use** the `rotation` property of [class_Node3D](../../classes/class_node3d.md#class_Node3D) nodes in Godot for games. It's there to be used mainly in the editor, for coherence with the 2D engine, and for simple rotations (generally just one axis, or even two in limited cases). As much as you may be tempted, don't use it.

Instead, there is a better way to solve your rotation problems.

### Introducing transforms

Godot uses the [class_Transform3D](../../classes/class_transform3d.md#class_Transform3D) datatype for orientations. Each [class_Node3D](../../classes/class_node3d.md#class_Node3D) node contains a `transform` property which is relative to the parent's transform, if the parent is a Node3D-derived type.

It is also possible to access the world coordinate transform via the `global_transform` property.

A transform has a [class_Basis](../../classes/class_basis.md#class_Basis) (transform.basis sub-property), which consists of three [class_Vector3](../../classes/class_vector3.md#class_Vector3) vectors. These are accessed via the `transform.basis` property and can be accessed directly by `transform.basis.x`, `transform.basis.y`, and `transform.basis.z`. Each vector points in the direction its axis has been rotated, so they effectively describe the node's total rotation. The scale (as long as it's uniform) can also be inferred from the length of the axes. A *basis* can also be interpreted as a 3x3 matrix and used as `transform.basis[x][y]`.
...
For more information on the mathematics of vectors and transforms, please read the [doc_vector_math](../math/vector_math.md#doc_vector_math) tutorials.
...
Is the enemy looking at the player? Use the dot product for this (see the [doc_vector_math](../math/vector_math.md#doc_vector_math) tutorial for an explanation of the dot product):
...
The [class_Quaternion](../../classes/class_quaternion.md#class_Quaternion) type reference has more information on the datatype (it
