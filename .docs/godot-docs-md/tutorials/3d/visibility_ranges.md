<span id="doc_visibility_ranges"></span>

## Visibility ranges (HLOD)

Along with [doc_mesh_lod](mesh_lod.md#doc_mesh_lod) and [doc_occlusion_culling](occlusion_culling.md#doc_occlusion_culling),
...
.. seealso::
    ...
    [doc_mesh_lod](mesh_lod.md#doc_mesh_lod) instead.
...
<span id="doc_visibility_ranges_fade_mode"></span>
...
- **Dependencies:** Uses alpha blending to smoothly fade between LOD levels. The
  ...
  [Visibility parent](visibility_ranges.md#doc_visibility_ranges_visibility_parent). It acts
  the same as **Self** if visibility ranges are used to perform non-hierarchical
  LOD.

<span id="doc_visibility_ranges_visibility_parent"></span>
...
> **Note:** The target of **Visibility Parent** *must* inherit from :ref:`class_GeometryInstance3D`. Despite its name, the **Visibility Parent** property *can* point to a node that is not a parent of the node in the scene tree. However, it is impossible to point **Visibility Parent** towards a child node, as this creates a dependency cycle which is not supported. You will get an error message in the Output panel if a dependency cycle occurs.
...
**Dependencies** [fade modes](visibility_ranges.md#doc_visibility_ranges_fade_mode).
...
- No visual glitches due to
  [transparency sorting issues](3d_rendering_limitations.md#doc_3d_rendering_limitations_transparency_sorting)
