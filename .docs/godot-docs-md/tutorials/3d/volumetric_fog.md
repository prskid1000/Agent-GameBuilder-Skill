<span id="doc_volumetric_fog"></span>
...
As described in [doc_environment_and_post_processing](environment_and_post_processing.md#doc_environment_and_post_processing), Godot supports
...
cannot be customized with [doc_fog_shader](../shaders/shader_reference/fog_shader.md#doc_fog_shader).
...
![](img/volumetric_fog_comparison.png)
...
![](img/volumetric_fog_lighting.png)
...
> **Note:** Volumetric fog can cause banding to appear on the viewport, especially at higher density levels. See :ref:`doc_3d_rendering_limitations_color_banding` for guidance on reducing banding.
...
- **Material:** The material used by the FogVolume. Can be either a
  built-in FogMaterial or a custom ShaderMaterial ([doc_fog_shader](../shaders/shader_reference/fog_shader.md#doc_fog_shader)).
...
- **Density Texture:** The 3D texture that is used to scale the member density
  ...
  [fog shader](../shaders/shader_reference/fog_shader.md#doc_fog_shader).
  You can import any image as a 3D texture by
  [changing its import type in the Import dock](../assets_pipeline/importing_images.md#doc_importing_images_changing_import_type).
...
> **Note:** NoiseTexture3D's **Color Ramp** affects FogMaterial density textures, but since only the texture's red channel is sampled, only the color ramp's red channel will affect the resulting density. However, using a color ramp will *not* tint the fog volume according to the texture. You would need to use a custom shader that reads a Texture3D to achieve this.
...
FogVolume nodes' appearance can be customized using [doc_fog_shader](../shaders/shader_reference/fog_shader.md#doc_fog_shader).
...
![](img/volumetric_fog_quad_mesh_texture.webp)
