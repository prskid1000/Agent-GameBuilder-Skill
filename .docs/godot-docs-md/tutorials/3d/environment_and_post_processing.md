<span id="doc_environment_and_post_processing"></span>
...
> **Note:** As of Godot 4, Environment *performance/quality* settings are defined in the project settings instead of in the Environment resource. This makes global adjustments easier, as you no longer have to tweak Environment resources individually to suit various hardware configurations. Note that most Environment performance/quality settings are only visible after enabling the **Advanced** toggle in the Project Settings.
### Environment

The [class_Environment](../../classes/class_environment.md#class_Environment) resource stores all the information required for
...
![](img/environment_preview_sun_sky_toggle.webp)
...
![](img/environment_preview_sun_sky_dialog.webp)
...
The [class_CameraAttributes](../../classes/class_cameraattributes.md#class_CameraAttributes) resource stores exposure and depth of field
...
> **Note:** Using a :ref:`class_CameraAttributesPhysical` on a Camera3D node will lock out FOV and aspect adjustments in that Camera3D, as field of view is adjusted in the CameraAttributesPhysical resource instead. If used in a WorldEnvironment, the CameraAttributesPhysical will not override any Camera3D in the scene.
...
![](img/environment_background1.webp)
...
- **Canvas** displays the 2D scene as a background to the 3D scene. This can be used
  ...
  [glow in 2D](environment_and_post_processing.md#doc_environment_and_post_processing_using_glow_in_2d).
- **Keep** does not draw any sky, keeping what was present on previous frames
  ...
  - **Camera Feed** displays a [class_CameraFeed](../../classes/class_camerafeed.md#class_CameraFeed) from a physical camera as the
...
create a custom [sky shader](../shaders/shader_reference/sky_shader.md#doc_sky_shader).
...
generate ambient light from [ReflectionProbe](global_illumination/reflection_probes.md#doc_reflection_probes),
[VoxelGI](global_illumination/using_voxel_gi.md#doc_using_voxel_gi) or [SDFGI](global_illumination/using_sdfgi.md#doc_using_sdfgi), as these
...
![](img/environment_ambient_comparison.webp)
...
[doc_reflection_probes](global_illumination/reflection_probes.md#doc_reflection_probes), they may not provide enough detail for all
...
<span id="doc_environment_and_post_processing_ssil"></span>
...
.. seealso::
    ...
    See [doc_using_sdfgi](global_illumination/using_sdfgi.md#doc_using_sdfgi) for instructions on setting up this global
    illumination technique.

![](img/environment_sdfgi.webp)

<span id="doc_environment_and_post_processing_glow"></span>
...
![](img/environment_glow_threshold.webp)
...
![](img/environment_glow_bloom.webp)
...
- **Replace** can be used to
  [blur the whole screen](environment_and_post_processing.md#doc_environment_and_post_processing_using_glow_to_blur_the_screen)
...
![](img/environment_and_post_processing_glow_scale_mode.webp)

<span id="doc_environment_and_post_processing_using_glow_in_2d"></span>
...
- If you want to maximize performance, you can leave HDR disabled for 2D
  ...
  - Enable glow, set the environment background mode to **Canvas** then decrease
    **Glow HDR Threshold** so that pixels that are not overbright will still
    ...
    [class_CanvasLayer](../../classes/class_canvaslayer.md#class_CanvasLayer) node. You can control which layers are affected by
...
when using the Canvas background mode, make them children of a [class_CanvasLayer](../../classes/class_canvaslayer.md#class_CanvasLayer)
...
> **Note:** This section refers to non-volumetric fog only. It is possible to use both non-volumetric fog and :ref:`doc_volumetric_fog` at the same time.
...
![](img/environment_fog_depth_height.webp)
...
> **Note:** Fog can cause banding to appear on the viewport, especially at higher density levels. See :ref:`doc_3d_rendering_limitations_color_banding` for guidance on reducing banding.
Volumetric Fog
~~~~~~~~~~~~~~
...
.. seealso::
  ...
  See [doc_volumetric_fog](volumetric_fog.md#doc_volumetric_fog) for documentation on setting up volumetric fog.
...
![](img/environment_adjustments.webp)
...
![](img/environment_adjustments_bcs.webp)
...
![](img/environment_adjustments_default_gradient.webp)

But creating custom ones will allow to map each channel to a different color:

![](img/environment_adjustments_custom_gradient.webp)
...
![](img/environment_adjustments_3d_lut_import.webp)
...
![](img/environment_adjustments_3d_lut_template.webp)
...
![](img/environment_adjustments_3d_lut_comparison.webp)

> **Note:** Adjustments and color correction are applied *after* tonemapping. This means the tonemapping properties defined above still have an effect when adjustments are enabled.
...
![](img/environment_mixed_blur.webp)
...
![](img/environment_hdr_autoexp.webp)
...
By combining Auto Exposure with [doc_environment_and_post_processing_glow](environment_and_post_processing.md#doc_environment_and_post_processing_glow)
...
.. seealso::
    ...
    See [doc_physical_light_and_camera_units](physical_light_and_camera_units.md#doc_physical_light_and_camera_units) if you wish to use real world
