...
2. Add the Godot engine header files as a dependency for your plugin library in `HEADER_SEARCH_PATHS`. You can find the setting inside the `Build Settings` tab:
    ...
    - Run SCons to generate headers. You can learn the process by reading [doc_compiling_for_ios](../../../engine_details/development/compiling/compiling_for_ios.md#doc_compiling_for_ios). You don't have to wait for compilation to complete to move forward as headers are generated before the engine starts to compile.
