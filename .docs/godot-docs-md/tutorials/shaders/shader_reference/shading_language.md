<span id="doc_shading_language"></span>
...
<span id="doc_shading_language_data_types"></span>
...
These types can also be put inside [arrays](shading_language.md#doc_shading_language_arrays)
or [structs](shading_language.md#doc_shading_language_structs), which are also usable as function parameters
...
per the [doc_shaders_style_guide](../shaders_style_guide.md#doc_shaders_style_guide). These asterisks are automatically
...
<span id="doc_shading_language_arrays"></span>
...
<span id="doc_shading_language_structs"></span>
...
> **Note:** Most operators that accept vectors or matrices (multiplication, division, etc) operate component-wise, meaning the function is applied to the first value of each vector and then on the second value of each vector, etc. Some examples: .. table:: :class: nowrap-col2 nowrap-col1 :widths: auto +---------------------------------------+------------------------------------------------------+ | Operation                             | Equivalent Scalar Operation                          | +=======================================+======================================================+ | ``vec3(4, 5, 6) + 2``                 | ``vec3(4 + 2, 5 + 2, 6 + 2)``                        | +---------------------------------------+------------------------------------------------------+ | ``vec2(3, 4) * vec2(10, 20)``         | ``vec2(3 * 10, 4 * 20)``                             | +---------------------------------------+------------------------------------------------------+ | ``mat2(vec2(1, 2), vec2(3, 4)) + 10`` | ``mat2(vec2(1 + 10, 2 + 10), vec2(3 + 10, 4 + 10))`` | +---------------------------------------+------------------------------------------------------+ The `GLSL Language Specification <http://www.opengl.org/registry/doc/GLSLangSpec.4.30.6.pdf>`_ says under section 5.10 Vector and Matrix Operations: With a few exceptions, operations are component-wise. Usually, when an operator operates on a vector or matrix, it is operating independently on each component of the vector or matrix, in a component-wise fashion. [...] The exceptions are matrix multiplied by vector, vector multiplied by matrix, and matrix multiplied by matrix. These do not operate component-wise, but rather perform the correct linear algebraic multiply.
### Flow control
...
<span id="doc_shading_language_varyings"></span>
...
<span id="doc_shading_language_uniforms"></span>
...
[data type](shading_language.md#doc_shading_language_data_types) except for `void` can be a uniform.
...
can set them [from code](shading_language.md#doc_shading_language_setting_uniforms_from_code).
...
<span id="doc_shading_language_global_uniforms"></span>
...
> **Note:** shader. While regular uniforms are defined outside of shader functions and are therefore the global scope of the shader, global uniforms are global to all shaders in the entire project (but within each shader, are also in the global scope).
...
[RenderingServer.global_shader_parameter_set](../../../classes/class_renderingserver.md#class_RenderingServer_method_global_shader_parameter_set)
method in a script:
...
[set_instance_shader_parameter](../../../classes/class_geometryinstance3d.md#class_GeometryInstance3D_method_set_instance_shader_parameter)
method on a node that inherits from [class_GeometryInstance3D](../../../classes/class_geometryinstance3d.md#class_GeometryInstance3D):
...
> **Note:** In GLSL versions before 4.0 (i.e. GLSL 3.3 and lower), you cannot directly index a texture array using a per-instance uniform, as sampler arrays can only be indexed by compile-time constant expressions. This affects shaders compiled with the Compatibility renderer. If you are affected, use the ``switch`` statement to select the texture:
```
```
...
<span id="doc_shading_language_setting_uniforms_from_code"></span>
...
> **Note:** in the shader. It must match *exactly* to the name of the uniform in the shader or else it will not be recognized.
...
+------------------------+-------------------------+------------------------------------------------------------+
| **samplerCube**        | **Cubemap**             | See [doc_importing_images_changing_import_type](../../assets_pipeline/importing_images.md#doc_importing_images_changing_import_type) for   |
...
> **Note:** will be thrown if the type does not match. Your shader will just exhibit undefined behavior. Specifically, this includes setting a GDScript int/float (64 bit) into a Godot shader language int/float (32 bit). This may lead to unintended consequences in cases where high precision is required.
Uniform limits
~~~~~~~~~~~~~~
...
- [Spatial shaders](spatial_shader.md#doc_spatial_shader)
- [Canvas item shaders](canvas_item_shader.md#doc_canvas_item_shader)
- [Particle shaders](particle_shader.md#doc_particle_shader)
- [Sky shaders](sky_shader.md#doc_sky_shader)
- [Fog shaders](fog_shader.md#doc_fog_shader)

### Built-in functions

A large number of built-in functions are supported, conforming to GLSL ES 3.0.
See the [Built-in functions](shader_functions.md#doc_shader_functions) page for details.
