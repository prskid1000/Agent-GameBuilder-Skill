<span id="doc_shader_preprocessor"></span>
...
> **Note:** The shader preprocessor is not available in :ref:`visual shaders <doc_visual_shaders>`. If you need to introduce preprocessor statements to a visual shader, you can convert it to a text-based shader using the **Convert to Shader** option in the VisualShader inspector resource dropdown. This conversion is a one-way operation; text shaders cannot be converted back to visual shaders.
### Directives
...
The [shading language](shading_language.md#doc_shading_language) supports runtime `if` statements:
...
menu option of the shader editor, or by creating a new [ShaderInclude](../../../classes/class_shaderinclude.md#class_ShaderInclude) resource
