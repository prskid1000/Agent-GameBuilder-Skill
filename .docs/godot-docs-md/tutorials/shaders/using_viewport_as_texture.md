<span id="doc_viewport_as_texture"></span>

## Using a SubViewport as a texture

### Introduction

This tutorial will introduce you to using the [SubViewport](../../classes/class_subviewport.md#class_SubViewport) as a
...
a [Camera3D](../../classes/class_camera3d.md#class_Camera3D), a [light source](../../classes/class_omnilight3d.md#class_OmniLight3D), a
[MeshInstance3D](../../classes/class_meshinstance3d.md#class_MeshInstance3D) with a [Primitive Mesh](../../classes/class_primitivemesh.md#class_PrimitiveMesh),
and applying a [StandardMaterial3D](../../classes/class_standardmaterial3d.md#class_StandardMaterial3D) to the mesh. The focus will be on using
the [SubViewport](../../classes/class_subviewport.md#class_SubViewport) to dynamically create textures that can be applied to the mesh.

In this tutorial, we'll cover the following topics:

- How to use a [SubViewport](../../classes/class_subviewport.md#class_SubViewport) as a render texture
- Mapping a texture to a sphere with equirectangular mapping
- Fragment shader techniques for procedural planets
- Setting a Roughness map from a [Viewport Texture](../../classes/class_viewporttexture.md#class_ViewportTexture)

### Setting up the scene

Create a new scene and add the following nodes exactly as shown below.

![](img/viewport_texture_node_tree.webp)

Go into the the MeshInstance3D and make the mesh a SphereMesh

### Setting up the SubViewport

Click on the [SubViewport](../../classes/class_subviewport.md#class_SubViewport) node and set its size to `(1024, 512)`. The
[SubViewport](../../classes/class_subviewport.md#class_SubViewport) can actually be any size so long as the width is double the
...
Next disable 3D. We will be using a [ColorRect](../../classes/class_colorrect.md#class_ColorRect) to render the surface, so
we don't need 3D either.

![](img/planet_new_viewport.webp)

Select the [ColorRect](../../classes/class_colorrect.md#class_ColorRect) and in the inspector set the anchors preset to `Full Rect`.
This will ensure that the [ColorRect](../../classes/class_colorrect.md#class_ColorRect) takes up the entire [SubViewport](../../classes/class_subviewport.md#class_SubViewport).

![](img/planet_new_colorrect.webp)

Next, we add a [Shader Material](../../classes/class_shadermaterial.md#class_ShaderMaterial) to the [ColorRect](../../classes/class_colorrect.md#class_ColorRect) (ColorRect > CanvasItem > Material > Material > `New ShaderMaterial`).

> **Note:** to shaders, all the code will be provided, so you should have no problem following along.
...
Now we have the basics of a [SubViewport](../../classes/class_subviewport.md#class_SubViewport) that we render to and we have a unique image that we can
apply to the sphere.

### Applying the texture

Now go into the [MeshInstance3D](../../classes/class_meshinstance3d.md#class_MeshInstance3D) and add a [StandardMaterial3D](../../classes/class_standardmaterial3d.md#class_StandardMaterial3D)
to it. No need for a special [Shader Material](../../classes/class_shadermaterial.md#class_ShaderMaterial) (although that would be a good idea
...
![](img/planet_new_viewport_texture.webp)
...
So now, when we render to our [SubViewport](../../classes/class_subviewport.md#class_SubViewport), it appears magically on the sphere. But there is an ugly
...
[StandardMaterial3D](../../classes/class_standardmaterial3d.md#class_StandardMaterial3D). It uses a projection technique called equirectangular
projection, which translates a spherical map onto a 2D plane.

> **Note:** spherical coordinates into Cartesian coordinates. Spherical coordinates map the longitude and latitude of the sphere, while Cartesian coordinates are, for all intents and purposes, a vector from the center of the sphere to the point.
...
[Viewport Texture](../../classes/class_viewporttexture.md#class_ViewportTexture) pointing to our planet texture [SubViewport](../../classes/class_subviewport.md#class_SubViewport).
...
of the [SubViewport](../../classes/class_subviewport.md#class_SubViewport) is opaque, the `alpha` channel of the
[Viewport Texture](../../classes/class_viewporttexture.md#class_ViewportTexture) is `1`, resulting in the planet texture being
drawn with slightly fainter colors and a `Roughness` value of `1` everywhere. To correct this, we
go into the [SubViewport](../../classes/class_subviewport.md#class_SubViewport) and enable the "Transparent Bg" property. Since we are now
...
Now the planet should look like it is reflecting light on the ocean but not the land. move around the [OmniLight3D](../../classes/class_omnilight3d.md#class_OmniLight3D)
in the scene so you can see the effect of the reflections on the ocean.

![](img/planet_ocean_reflect.webp)

And there you have it. A procedural planet generated using a [SubViewport](../../classes/class_subviewport.md#class_SubViewport).
