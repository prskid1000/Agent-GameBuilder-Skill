<span id="doc_advanced_postprocessing"></span>
...
generally and, in particular, with the methods outlined in the [custom post-processing tutorial](custom_postprocessing.md#doc_custom_postprocessing).
...
To get around the limitation on using the depth buffer, use a [MeshInstance3D](../../classes/class_meshinstance3d.md#class_MeshInstance3D)
with a [QuadMesh](../../classes/class_quadmesh.md#class_QuadMesh) primitive. This allows us to use a
...
> **Note:** which implicitly assumed the clip-space near plane was at ``0.0``. That code is now incorrect and will not work in versions 4.3+ as we use a "reversed-z" depth buffer now where the near plane is at ``1.0``.
...
> **Note:** possible when reading from the current viewport. The depth texture cannot be accessed from another viewport to which you have rendered.
...
Set the Mesh in the MeshInstance3D to an [ArrayMesh](../../classes/class_arraymesh.md#class_ArrayMesh). An
...
> **Note:** Recall, NDC run from ``-1.0`` to ``1.0`` in both the ``x`` and ``y`` directions. This makes the screen ``2`` units wide and ``2`` units tall. In order to cover the entire screen with a single triangle, use a triangle that is ``4`` units wide and ``4`` units tall, double its height and width.
