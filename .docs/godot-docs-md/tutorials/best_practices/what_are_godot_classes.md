<span id="doc_what_are_godot_classes"></span>
...
The engine provides built-in classes like [Node](../../classes/class_node.md#class_Node). You can extend those to create
...
Attaching a [Script](../../classes/class_script.md#class_Script) to your object extends the methods, properties, and signals
available from the `ClassDB`.

> **Note:** Even scripts that don't use the ``extends`` keyword implicitly inherit from the engine's base :ref:`RefCounted <class_RefCounted>` class. As a result, you can instantiate scripts without the ``extends`` keyword from code. Since they extend ``RefCounted`` though, you cannot attach them to a :ref:`Node <class_Node>`.
### Scenes
