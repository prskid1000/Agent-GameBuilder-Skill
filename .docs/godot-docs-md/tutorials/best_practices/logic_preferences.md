<span id="doc_logic_preferences"></span>
...
[ResourceLoader.load(path)](../../classes/class_resourceloader.md#class_ResourceLoader_method_load) which is
...
3. If one wishes only to 'import' another class resource (script or scene),
   ...
   2. If the script requires a great many dependencies, and one does not wish
      ...
      [RefCounted](../../classes/class_refcounted.md#class_RefCounted)-extending type, will cause the
...
please see the ["Change scenes manually"](../scripting/change_scenes_manually.md#doc_change_scenes_manually)
documentation.
